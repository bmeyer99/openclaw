...
export async function prepareSlackMessage(params: {
  ...
  const textWithId = `${rawBody}\n[slack message id: ${message.ts} channel: ${message.channel}${threadTs ? ` thread_ts: ${threadTs}` : ""}]`;
  ...
  if (isThreadReply && threadTs) {
    ...
    if (starter?.text) {
      ...
      const starterWithId = `${starter.text}\n[slack message id: ${starter.ts ?? threadTs} channel: ${message.channel} thread_ts: ${threadTs}]`;
