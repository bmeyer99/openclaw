...
type AgentDefaultsConfig = {
  ...
  maxConcurrent?: number; /** MCP context hook — calls an MCP tool to build context before each agent turn. */
  contextHook?: {
    /** Enable the context hook. Default: false. */enabled?: boolean; /** MCP server name (e.g., "my-rag-server"). */
    mcpServer?: string; /** MCP tool name to call. Default: "build_agent_context". */
    mcpTool?: string; /** Timeout in milliseconds. Default: 10000. */
    timeoutMs?: number; /** Token budget for injected context. Default: 60000. */
    budgetTokens?: number;
...
type LifecycleReactionsConfig = {
  /** Emoji when message is received (default: uses ackReaction if set). */received?: string; /** Emoji when message is queued waiting for processing. */
  queued?: string; /** Emoji when model is actively processing. */
  processing?: string; /** Emoji when response is complete (your turn). */
  complete?: string;
};
type MessagesConfig = {
   ...
   */
  lifecycleReactions?: LifecycleReactionsConfig; /** Text-to-speech settings for outbound replies. */
  tts?: TtsConfig;
};
